...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CHelveticumAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/helveticum/helveticum/pull/10340, but that change has been
    // postponed until after https://github.com/helveticum/helveticum/pull/10244 to
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-helveticum-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CHelveticumAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CHelveticumAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CHelveticumAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CHelveticumAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CHelveticumAddress(address).ToString());
}
 ...
 * https://helveticumtalk.org/?topic=54527, or
 * https://github.com/helveticum/helveticum/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CHelveticumAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CHelveticumAddress(dest).ToString(), key);
}
