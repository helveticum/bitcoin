...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s helveticum-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  helveticum-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded helveticum transaction") + "\n" +
              "  helveticum-tx [options] -create [commands]   " + _("Create hex-encoded helveticum transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CHelveticumAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CHelveticumAddress addr(scriptPubKey);
    ...
    if (bScriptHash) {
        ...
        CHelveticumAddress redeemScriptAddr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded helveticum transaction
