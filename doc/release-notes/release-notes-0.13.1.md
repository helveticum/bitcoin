Helveticum Core version 0.13.1 is now available from:

  <https://helveticum.org/bin/helveticum-core-0.13.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://helveticumcore.org/en/list/announcements/join/>
...
updates will be released anymore. Without security updates, using a helveticum
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Helveticum Core
randomly crashing on Windows XP. It is [not clear](https://github.com/helveticum/helveticum/issues/7681#issuecomment-217439891)
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Helveticum wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Helveticum.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Helveticum block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of helveticums they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of helveticums being spent is exactly
...
- **Increased security for multisig:** Helveticum addresses (both P2PKH addresses
  ...
  believed to be provided by Helveticum's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Helveticum paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Helveticum's current ledger (called the UTXO set),
  ...
  note that Helveticum Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Helveticum users to individually opt-in to almost any change in the Helveticum
  Script language when those users receive new transactions.  Features
  currently being researched by Helveticum Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Helveticum community members are studying several other improvements
...
[Segwit FAQ]: https://helveticumcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://helveticumcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/helveticum/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/helveticum/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/helveticum/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/helveticum/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://helveticumcore.org/en/2016/06/08/version-bits-miners-faq/
...
Since Helveticum Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/helveticum/bips/blob/master/bip-0147.mediawiki
...
- `helveticum-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `helveticum-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in helveticumconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.helveticumstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libhelveticumconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_helveticum by itself (luke-jr)
...
- #8829 `2a8bca4` Add helveticum-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` helveticum-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_helveticumds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name helveticum.qrc  (helveticumsSG)
...
- #8817 `bcf3806` update helveticum-tx to output witness data (jnewbery)
