#!/bin/bash
# Copyright (c) 2013 The Helveticum Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Helveticum database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Helveticum datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Helveticum datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Helveticum 0.7 datadir."
    ;;
  3)
    echo "Detected Helveticum pre-0.8 datadir."
    ;;
  4)
    echo "Detected Helveticum 0.8 datadir."
    ;;
esac
