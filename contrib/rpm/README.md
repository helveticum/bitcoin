...
The RPM spec file provided here is for Helveticum-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://helveticum.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Helveticum git repository](https://github.com/helveticum/helveticum) but are not part of
...
Currently only `Source10` is used, for the example `helveticum.conf` file.
...
in the helveticum git repository. At present only an SVG version of the Helveticum
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the helveticum
...
needed when 0.12.1 is released, a proper fix is already in the Helveticum git
master branch.
...
idea to build Helveticum Core against a static version of that library compiled
...
The `helveticum.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `helveticum-server` package. At this point, most RPM based
...
The `helveticum-server` package should have SELinux support. How to properly do
...
Helveticum-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/helveticum/). He has been
packaging Helveticum for Fedora at least since 2012.
...
4. I renamed the `helveticum` package that contains the Qt GUI to `helveticum-core` as
that appears to be how the general population refers to it, in contrast to
`helveticum-xt` or `helveticum-classic`. I wanted to make sure the general population
